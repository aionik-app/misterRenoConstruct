...
import type { AdminConfig, LoginAttempt, AdminSession } from "@/types/site-config"

const ADMIN_CONFIG_PATH = path.join(process.cwd(), "data", "admin-config.json")
const LOGIN_ATTEMPTS_PATH = path.join(process.cwd(), "data", "login-attempts.json")
...
export async function saveLoginAttempts(attempts: LoginAttempt[]): Promise<void> {
  await fs.writeFile(LOGIN_ATTEMPTS_PATH, JSON.stringify(attempts, null, 2), "utf8")
}
...
// Save admin sessions
export async function saveAdminSessions(sessions: AdminSession[]): Promise<void> {
...
export async function isIPLockedOut(ip: string): Promise<boolean> {
  ...
  const recentAttempts = attempts.filter(
    (attempt) =>
      attempt.ip === ip &&
      !attempt.success &&
      new Date(attempt.timestamp).getTime() > Date.now() - config.lockoutDuration * 60 * 1000,
  )
...
export async function recordLoginAttempt(ip: string, success: boolean): Promise<void> {
  ...
  const recentAttempts = attempts.filter((attempt) => new Date(attempt.timestamp).getTime() > oneDayAgo)
...
export async function validateAdminSession(token: string, ip: string): Promise<boolean> {
  const sessions = await loadAdminSessions()
...
export async function cleanupExpiredSessions(): Promise<void> {
  const sessions = await loadAdminSessions()
  const validSessions = sessions.filter((session) => new Date(session.expiresAt).getTime() > Date.now())
  await saveAdminSessions(validSessions)
}
...
// Change admin code
export async function changeAdminCode(currentCode: string, newCode: string, ip: string): Promise<boolean> {
  const config = await loadAdminConfig()
