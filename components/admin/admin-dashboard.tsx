    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Alert variant="destructive" className="max-w-md">
          <AlertDescription>Impossible de charger la configuration</AlertDescription>
        </Alert>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-muted/30">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div>
              ...
              <p className="text-muted-foreground">Gestion du contenu du site</p>
            </div>

            <div className="flex items-center gap-4">
              <Button variant="outline" asChild>
                <a href="/" target="_blank" className="flex items-center gap-2" rel="noreferrer">
      ...
      {saveMessage && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
          ...
          >
            <AlertDescription
              className={saveMessage.includes('succès') ? 'text-green-800' : 'text-red-800'}
            >
              {saveMessage}
            </AlertDescription>
          </Alert>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="p-6">
              <TabsList className="grid grid-cols-6 lg:grid-cols-13 gap-2 h-auto bg-transparent">
                {tabs.map((tab) => (
                  <TabsTrigger
                    key={tab.id}
                    value={tab.id}
          ...
          <TabsContent value="testimonials">
            <TestimonialsEditor config={config} onUpdate={updateConfig} />
          </TabsContent>

          <TabsContent value="pricing">
            <PricingEditor config={config} onUpdate={updateConfig} />
          </TabsContent>
