...
import type { SiteConfig, SocialLink, QuickLink } from "@/types/site-config"
...
export function FooterEditor({ config, onUpdate }: FooterEditorProps) {
  const updateFooter = (field: string, value: string | SocialLink[] | QuickLink[]) => {
    onUpdate({
      footer: {
        ...config.footer,
        [field]: value,
  ...
  const removeSocialLink = (index: number) => {
    const updatedLinks = config.footer.socialLinks.filter((_, i) => i !== index)
    updateFooter("socialLinks", updatedLinks)
  }
    ...
    <div className="space-y-6">
      <Card className="border-0 shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Configurez le pied de page de votre site</CardDescription>
        </CardHeader>
      ...
      <Card className="border-0 shadow-lg">
        <CardHeader>
          <div className="flex justify-between items-center">
            ...
            <Button onClick={addSocialLink} size="sm" className="flex items-center gap-2">
        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription>Liens de navigation dans le footer</CardDescription>
            </div>
        ...
        <CardContent className="space-y-4">
          {config.footer.quickLinks.map((link, index) => (
            <div key={index} className="flex gap-4 items-end">
              ...
              <Button variant="destructive" size="sm" onClick={() => removeQuickLink(index)}>
                <Trash2 className="h-4 w-4" />
              </Button>
            </div>
          ))}
