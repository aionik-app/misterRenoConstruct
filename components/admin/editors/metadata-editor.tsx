...
export function MetadataEditor({ config, onUpdate }: MetadataEditorProps) {
  ...
  const addKeyword = () => {
    if (newKeyword.trim() && !config.metadata.keywords.includes(newKeyword.trim())) {
      onUpdate({
        metadata: {
          ...config.metadata,
  ...
  return (
    <Card className="border-0 shadow-lg">
      <CardHeader>
        ...
        <CardDescription>
          Configurez les informations de base du site pour le référencement et les réseaux sociaux
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          />
          <p className="text-sm text-muted-foreground">
            Résumé du site affiché dans les résultats de recherche (150-160 caractères recommandés)
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">Mots-clés importants pour le référencement de votre site</p>
        </div>
      </CardContent>
    </Card>
  )
