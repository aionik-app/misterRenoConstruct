...
export function SecurityEditor() {
  ...
  const handleChangeCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage("Code d'accès modifié avec succès. Vous allez être déconnecté.")
    ...
    <div className="space-y-6">
      <Card className="border-0 shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Gérez les paramètres de sécurité de votre interface d'administration</CardDescription>
        </CardHeader>
      </Card>
      ...
      <Card className="border-0 shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Modifiez votre code d'accès administrateur pour renforcer la sécurité</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleChangeCode} className="space-y-4">
            ...
            <Button type="submit" disabled={isLoading} className="w-full">
              {isLoading ? "Modification en cours..." : "Changer le code d'accès"}
            </Button>
          </form>

          <div className="mt-6 p-4 bg-muted/50 rounded-lg">
            <h4 className="font-semibold text-sm mb-2">Conseils de sécurité :</h4>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 bg-muted/50 rounded-lg">
              <h4 className="font-semibold text-sm mb-2">Protection contre les attaques</h4>
              <p className="text-sm text-muted-foreground">
                Limitation à 5 tentatives de connexion par IP, avec blocage temporaire de 15 minutes.
              </p>
            </div>
            <div className="p-4 bg-muted/50 rounded-lg">
              <h4 className="font-semibold text-sm mb-2">Sessions sécurisées</h4>
              <p className="text-sm text-muted-foreground">
