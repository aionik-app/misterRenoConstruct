...
export function AboutEditor({ config, onUpdate }: AboutEditorProps) {
  ...
  const addFeature = () => {
    if (newFeature.trim() && !config.about.features.includes(newFeature.trim())) {
  ...
  return (
    <Card className="border-0 shadow-lg">
      <CardHeader>
        ...
        <CardDescription>Présentez votre entreprise et votre expertise</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Image représentant votre équipe ou votre travail (format recommandé: JPG, taille: 600x400px)
          </p>
          {config.about.image && (
            <div className="mt-2">
              <img
                ...
                className="w-full max-w-md h-48 object-cover border border-border rounded"
                onError={(e) => {
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            Points forts et avantages de votre entreprise (ex: "15 ans d'expérience", "Équipe certifiée")
          </p>
        </div>
      </CardContent>
    </Card>
