...
export async function POST(request: NextRequest) {
  try {
    ...
    if (newCode.length < 6) {
      return NextResponse.json({ error: "Le nouveau code doit contenir au moins 6 caractères" }, { status: 400 })
    }

    const success = await changeAdminCode(currentCode, newCode, ip)

    if (success) {
      const response = NextResponse.json({ success: true, message: "Code d'accès modifié avec succès" })
